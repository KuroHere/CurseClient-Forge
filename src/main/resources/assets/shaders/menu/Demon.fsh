/*
 * Original shader from: https://www.shadertoy.com/view/cljGR3
 */
...
// shadertoy emulation dickkkkkkkkkkkkkkk
#define iTime time
...
//
//
// More anme girls on shadertoy:
//
// Code:002 by Pidhorskyi
// https://www.shadertoy.com/view/tlsfzf

// For real
const vec3 BLOOD_COLOR = vec3(255, 255, 255) / 255.;
...
const vec3 HIGHLIGHT_COLOR = vec3(240, 38, 18) / 255. * 1.2;
...
#define LayerStroked(d, color, stroke) alpha_blend_inplace(final_color, render_stroked(d, color, stroke))
#define LayerStrokedMask(d, color, stroke, mask) alpha_blend_inplace(final_color, render_stroked_masked(d, color, stroke, mask))
