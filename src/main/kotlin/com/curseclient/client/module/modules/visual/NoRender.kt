...
object NoRender : Module(
    ...
    init {
        ...
        safeListener<ArmorRenderEvent> { event ->
            if (event.slot === EntityEquipmentSlot.HEAD && head) {
                event.cancel()
            } else if (event.slot === EntityEquipmentSlot.CHEST && chestplate) {
                event.cancel()
            } else if (event.slot === EntityEquipmentSlot.LEGS && leggings) {
                event.cancel()
            } else if (event.slot === EntityEquipmentSlot.FEET && boots) {
                event.cancel()
            }
