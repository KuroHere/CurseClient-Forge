...
object Compass: DraggableHudModule(
    ...
    private val dynamic by setting("Moving", false)

    //Theme1
    private val lineOption by setting("Option", Line.Line, { theme == Theme.Theme1 })
    ...
    override fun onRender() {
        ...
        progress = MathUtils.lerp(progress, dynamic.toInt().toDouble(), GLUtils.deltaTimeDouble() * 5.0 * 1.5)
        ...
        if (isInputting()) {
            ...
            offsetX = fast(offsetX.toFloat(), (sin(time) * swingAmount).toFloat(), 5.0f).toDouble()
            offsetY = fast(offsetY.toFloat(), (cos(time) * swingAmount).toFloat(), 5.0f).toDouble()
        } else {
        ...
        GL11.glPushMatrix()
        GL11.glTranslatef((offsetX.toFloat() * progress).toFloat(), (offsetY.toFloat() * progress).toFloat(), 0f)
        when (theme) {
            Theme.Theme1 -> firstTheme(pos1)
            Theme.Theme2 -> secondTheme(pos1, pos2)
        }
        GL11.glPopMatrix()
    }

    private fun firstTheme(pos1: Vec2d) {
        firstLeftTheme(pos1)
        firstRightTheme(pos1)
    }

    private fun firstLeftTheme(pos1: Vec2d) {
        drawCompass(pos1, true)
    }

    private fun firstRightTheme(pos1: Vec2d) {
        drawCompass(pos1, false)
    }
    ...
    private fun drawCompass(
        ...
        for ((index, direction) in compassDirections.withIndex()) {
            ...
            RenderUtils2D.drawLine(
                ...
                (pos.y + frb.getHeight(scale)).toFloat() + 0.7f,
                (pos.x - rotationYaw + getWidth() / 2 + offsets[index] - frb.getStringWidth(direction, scale / 2)).toFloat(),
                (pos.y + frb.getHeight(scale)).toFloat() + 0.7f,
                2.2f,
                Color.WHITE.rgb
