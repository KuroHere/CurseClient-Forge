...
object CrystalAura : Module(
    ...
    // General
    private val targetReach by setting("TargetReach", 12.0, 3.0, 30.0, 0.5, { page == Page.General })
    private val targetPriority by setting("TargetPriority", TargetPriority.Health, { page == Page.General })
    ...
    private val placeRotate by setting("PlaceRotate", true, { page == Page.General })
    private val explodeRotate by setting("ExplodeRotate", true, { page == Page.General })
    private val yawSpeed by setting("YawSpeed", 45.0, 1.0, 180.0, 1.0, { page == Page.General && (placeRotate || explodeRotate) })
    private val pitchSpeed by setting("PitchSpeed", 25.0, 1.0, 90.0, 1.0, { page == Page.General && (placeRotate || explodeRotate) })
    private val maxYawDiff by setting("MaxYawDiff", 3.0, 1.0, 180.0, 1.0, { page == Page.General && (placeRotate || explodeRotate) })
    private val maxPitchDiff by setting("MaxPitchDiff", 5.0, 1.0, 90.0, 1.0, { page == Page.General && (placeRotate || explodeRotate) })
    private val keepRotationTicks by setting("KeepRotationTicks", 5.0, 2.0, 20.0, 1.0, { page == Page.General && (placeRotate || explodeRotate) })
    private val hotbarMode by setting("HotbarMode", HotbarMode.Swap, { page == Page.General })
    private val swapBack by setting("SwapBack", true, { page == Page.General && hotbarMode == HotbarMode.Swap })
    private val keepHotbarTicks by setting("KeepHotbarTicks", 10.0, 2.0, 40.0, 1.0, { page == Page.General && hotbarMode != HotbarMode.None })
    private val swapPause by setting("SwapPause", 0.0, 0.0, 1000.0, 5.0, { page == Page.General })
    private val pauseWhileDigging by setting("PauseWhileDigging", true, { page == Page.General })
    private val pauseWhileEating by setting("PauseWhileEating", true, { page == Page.General })
    private val ignoreOffhand by setting("IgnoreOffhand", true, { page == Page.General && pauseWhileEating })
    ...
    private val placeWallReach by setting("WallReach(Place)", 3.0, 2.0, 8.0, 0.05, { page == Page.Place && place })
    private val minDamagePlace by setting("Min Damage(Place)", 4.0, 1.0, 20.0, 0.25, { page == Page.Place && place })
    private val maxSelfDamagePlace by setting("MaxSelfDamage(Place)", 5.0, 2.0, 36.0, 0.5, { page == Page.Place && place })
    private val antiSuicidePlace by setting("AntiSuicide(Place)", 0.0, 0.0, 10.0, 0.25, { page == Page.Place && place })
    private val placeDelay by setting("Delay(Place)", 10.0, 0.0, 1000.0, 5.0, { page == Page.Place && place })
    private val maxPlaceAttempts by setting("MaxAttempts Place)", 50.0, 1.0, 50.0, 1.0, { page == Page.Place && place })
    private val placeRetryDelay by setting("RetryDelay(Place)", 1000.0, 200.0, 5000.0, 10.0, { page == Page.Place && place })
    private val placeSwing by setting("SwingMode(Place)", SwingMode.Client, { page == Page.Place && place })
    private val multiPlacing by setting("Multi-Placing", false, { page == Page.Place && place })
    private val maxCrystals by setting("MaxCrystals", 2.0, 2.0, 10.0, 1.0, { page == Page.Place && place && multiPlacing })
    private val packetVec by setting("PacketOffset Mode", HitVecMode.BySide, { page == Page.Place && place })
    private val packetOffset by setting("PacketOffset", 0.5, 0.0, 1.0, 0.05, { page == Page.Place && place && packetVec == HitVecMode.Custom })
    private val newPlace by setting("1.13 Crystal", false, { page == Page.Place && place })
    private val placeSync by setting("PlaceSync", false, { page == Page.Place && place })
    private val extraPlace by setting("ExtraPlace", false, { page == Page.Place && place })
    ...
    private val minDamageExplode by setting("MinDamage(Explode)", 4.0, 1.0, 20.0, 0.25, { page == Page.Explode && explode })
    private val maxSelfDamageExplode by setting("MaxSelfDamage(Explode)", 5.0, 2.0, 36.0, 0.5, { page == Page.Explode && explode })
    private val antiSuicideExplode by setting("AntiSuicide(Explode)", 0.0, 0.0, 10.0, 0.25, { page == Page.Explode && explode })
    private val explodeDelay by setting("Delay(Explode)", 10.0, 0.0, 1000.0, 5.0, { page == Page.Explode && explode })
    private val maxExplodeAttempts by setting("MaxAttempts(Explode)", 50.0, 1.0, 50.0, 1.0, { page == Page.Explode && explode })
    private val explodeRetryDelay by setting("RetryDelay(Explode)", 1000.0, 200.0, 5000.0, 10.0, { page == Page.Explode && explode })
    private val explodeSwing by setting("SwingMode(Explode)", SwingMode.Client, { page == Page.Explode && explode })
    private val crystalHeight by setting("CrystalHeight", 0.00, 0.0, 1.0, 0.02, { page == Page.Explode && explode && explodeRotate })
    private val spawnExplode by setting("SpawnExplode", false, { page == Page.Explode && explode })

    // Extras
    private val forcePlace by setting("ForcePlace", false, { page == Page.Extras })
    private val forcePlaceMinDamage by setting("FP MinDamage", 1.0, 0.1, 5.0, 0.05, { page == Page.Extras && forcePlace })
    private val forcePlaceMaxTargetHealth by setting("FP HealthThreshold", 4.0, 2.0, 20.0, 0.1, { page == Page.Extras && forcePlace })
    private val forcePlaceMaxTargetDura by setting("FP ArmorDuraThreshold", 5.0, 2.0, 100.0, 1.0, { page == Page.Extras && forcePlace })
    private val antiSurround by setting("AntiSurround", false, { page == Page.Extras })
    private val antiSurroundPickaxeOnly by setting("AS PickaxeOnly", true, { page == Page.Extras && antiSurround })

    // Multithreading
    private val damageCalculationDelay by setting("CalculationThreadDelay", 50.0, 25.0, 100.0, 1.0, { page == Page.Multithreading })
    private val interactDelay by setting("InteractThreadDelay", 15.0, 1.0, 50.0, 1.0, { page == Page.Multithreading })
    @Suppress("UNUSED") private val reloadSetting by setting("ReloadThreads", { calculationThread.reload(); interactThread.reload() }, { page == Page.Multithreading })

    // Render
    private val render by setting("Render", true, { page == Page.Render })
    private val boxSize by setting("BoxSize", 1.0, 0.2, 1.0, 0.05, { page == Page.Render && render })
    private val moveSpeed by setting("MoveSpeed", 1.0, 0.5, 3.0, 0.1, { page == Page.Render && render })
    private val lineWidth by setting("LineWidth", 1.0, 1.0, 5.0, 0.1, { page == Page.Render && render })
    private val fadeDelay by setting("FadeDelay", 500.0, 0.0, 2000.0, 50.0, { page == Page.Render && render })
