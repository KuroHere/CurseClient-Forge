...
public class ShaderUtils implements Helper {
    ...
    public ShaderUtils(String fragmentShadersrc, boolean notUsed) {
        int program = glCreateProgram();
        int fragmentShaderID = createShader(new ByteArrayInputStream(fragmentShadersrc.getBytes()), GL_FRAGMENT_SHADER);
        int vertexShaderID = 0;
        try {
            vertexShaderID = createShader(mc.getResourceManager().getResource(new ResourceLocation("/assets/shaders/vertexGui.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    public ShaderUtils(String fragmentShaderLoc) {
        this(fragmentShaderLoc, "/assets/shaders/vertexGui.vsh");
    }
